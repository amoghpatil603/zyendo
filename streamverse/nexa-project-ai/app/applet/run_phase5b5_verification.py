...
def run_phase5b5_verification():
    ...
    print("Waiting for server readiness at http://localhost:3000...")
    for retry in range(20):
        ...
        raise RuntimeError("Server failed to become ready at http://localhost:3000")
    ...
    def send_queued_request(prompt_id):
        try:
            ...
            print(f"Queue task {prompt_id} error: {e}")
    ...
    if successful_queued != 5:
        print("Queue Test details:", queue_results)
    ...
    with open("/app/applet/phase5b5_resource_report.json", "w") as f:
        json.dump(resource_report, f, indent=2)

    with open("/app/applet/phase5b5_stress_test.json", "w") as f:
        json.dump(stress_report, f, indent=2)

    with open("/app/applet/phase5b5_watchdog_report.json", "w") as f:
        json.dump(watchdog_report, f, indent=2)
...
"""
    ...
    with open("/app/applet/phase5b5_final_report.md", "w") as f:
        f.write(final_md)
