...
import { applyRecommendationFeedbackToDna } from "@/lib/dna/feedback";
...
export async function recomputeDna(): Promise<DnaActionResult> {
  ...
  const [{ data: rows, error: fetchError }, { data: feedback, error: feedbackError }] = await Promise.all([
    supabase.from("watchlist").select("media_type, media_id").eq("user_id", user.id),
    supabase.from("recommendation_feedback").select("media_type, media_id, reaction").eq("user_id", user.id),
  ]);

  if (fetchError || feedbackError) return { ok: false, error: "failed" };

  // 2. Compute DNA from the watchlist.
  const result = await computeDnaFromMediaIds(
    applyRecommendationFeedbackToDna(
      (rows ?? []).map((r: { media_type: string; media_id: string }) => ({ mediaType: r.media_type, mediaId: r.media_id })),
      (feedback ?? []) as { media_type: string; media_id: string; reaction: "love" | "like" | "dislike" }[],
    ),
  );
  ...
  const dbRow: DnaRow = {
    ...
    preferred_runtime_range: result.preferredRuntimeRange
      ? { min: result.preferredRuntimeRange[0], max: result.preferredRuntimeRange[1] }
      : null,
    mood_tags: result.moodTags.map((m) => m.tag),
    is_public: false,
...
export async function getMyDna(): Promise<EntertainmentDna | null> {
  ...
  return {
    ...
    preferredRuntimeRange: row.preferred_runtime_range
      ? [row.preferred_runtime_range.min, row.preferred_runtime_range.max]
      : null,
    moodTags: (row.mood_tags ?? []).map((tag) => ({ tag, weight: 1 })),
