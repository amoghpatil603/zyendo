...
// ---------------------------------------------------------------------------
// V2 Attributes: Release Decades & Time of Day
// ---------------------------------------------------------------------------

function computeReleaseDecades(items: MediaItem[]): string[] {
  const map = new Map<string, number>();
  for (const item of items) {
    if (item.releaseDate && item.releaseDate.length >= 4) {
      ...
      if (!isNaN(year)) {
        ...
        map.set(decadeStr, (map.get(decadeStr) ?? 0) + 1);
      }
    }
  }
  return [...map.entries()]
...
function computeTimeOfDayHabits(viewTimestamps: string[]): string[] {
  const map = new Map<string, number>();
...
export function computeDna(input: DnaInput): DnaResult {
  const { items, viewTimestamps = [] } = input;
  ...
  return {
    ...
    releaseDecades: computeReleaseDecades(items),
    timeOfDayHabits: computeTimeOfDayHabits(viewTimestamps),
  };
}
...
export async function computeDnaFromMediaIds(
  rows: { mediaType: string; mediaId: string; createdAt?: string }[],
): Promise<DnaResult> {
  const items: MediaItem[] = [];
  const viewTimestamps: string[] = [];
  ...
  for (const row of rows) {
    try {
      ...
      if (detail) {
        ...
        if (row.createdAt) viewTimestamps.push(row.createdAt);
      }
    } catch {
      continue;
    }
  ...
  return computeDna({ items, viewTimestamps });
}