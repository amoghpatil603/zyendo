      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {episodes.map((ep) => (
            <li
              key={ep.id}
              className="flex gap-3 sm:gap-4 rounded-xl border border-border/60 bg-card/50 p-3"
            >
              <div className="relative aspect-video w-32 sm:w-40 shrink-0 overflow-hidden rounded-lg bg-muted">
                {ep.stillUrl ? (
                  <Image
                    src={ep.stillUrl}
                    alt={ep.name}
