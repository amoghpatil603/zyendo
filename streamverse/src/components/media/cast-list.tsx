      ...
      <div className="no-scrollbar flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2">
        {people.map((person, i) => (
          <figure
            ...
            className="w-24 shrink-0 snap-start text-center sm:w-28"
          >
            <div className="relative mx-auto size-20 sm:size-24 overflow-hidden rounded-full border border-border/60 bg-muted">
              {person.imageUrl ? (
                <Image
                  src={person.imageUrl}
                  alt={person.name}
