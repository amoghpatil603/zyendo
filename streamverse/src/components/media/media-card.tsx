...
export const MediaCard = memo(function MediaCard({
  ...
  const TypeIcon = item.type === "tv" ? Tv : item.type === "person" ? User : Film;

  return (
    <Link
      href={mediaHref(item)}
      className={cn(
        "group relative block overflow-hidden rounded-xl border border-border/60 bg-card outline-none transition-all duration-350 hover:-translate-y-1.5 hover:border-primary/40 hover:shadow-2xl hover:shadow-primary/5 focus-visible:ring-2 focus-visible:ring-ring",
        className,
      )}
    >
